...
function EditReport() {
        ...
        const fetchDropdowns = async () => {
            try {
                const [catRes, distRes] = await Promise.all([
                    axios.get("http://localhost:5000/api/user/categories", { headers: { Authorization: `Bearer ${token}` } }),
                    axios.get("http://localhost:5000/api/user/districts", { headers: { Authorization: `Bearer ${token}` } }),
                ]);
    ...
    useEffect(() => {
        ...
        fetchAreas();
    }, [formData.districtId, token, districtChanged]);
